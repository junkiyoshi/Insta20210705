...
void ofApp::draw() {
	...
	auto target_list = vector<glm::vec2>();
	for (int i = 0; i < 3; i++) {
	
		auto target = glm::vec2(ofMap(ofNoise(ofRandom(1000), ofGetFrameNum() * 0.005), 0, 1, -450, 450), ofMap(ofNoise(ofRandom(1000), ofGetFrameNum() * 0.005), 0, 1, -450, 450));
	...
	for (int x = -300; x <= 300; x += 30) {
		...
		for (int y = -300; y <= 300; y += 30) {
			...
			for (int i = 0; i < target_list.size(); i++) {
				...
				auto distance = glm::distance(glm::vec2(x, y), target_list[i]);
			...
			fill_color.setHsb(ofMap(min_index, 0, target_list.size(), 0, 255), 180, 255);
			this->draw_arrow(glm::vec2(x, y), target_list[min_index], 18, fill_color, ofColor(39));
		}
	}
}
